...
myStartupHook = do
    ...
    spawn "variety"
    spawn "volumeicon"
    spawn "unclutter -idle 10"
    spawn "flameshot"
    spawn "xfce.xfce4-power-manager"
    spawn "xfce.xfce4-notifyd"
    spawn "nm-tray"
    spawn "xsettingsd"
    spawn "pasystray"
    spawn "redshift"
...
myWorkspaces :: [String]
myWorkspaces    = ["Work","Personal","Terminal","4:code","5:doc","6:media","7","8","9","10"]
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  , ((controlMask .|. mod1Mask , xK_a ), spawn $ "arandr")
 ...
 -- , ((controlMask .|. mod1Mask , xK_e ), spawn $ "emacsclient -nc -s 'spacemacs'")
  ...
  -- ALT + SHIFT KEYS
  , ((mod1Mask .|. shiftMask, xK_n), spawn $ "variety -n" )
  , ((mod1Mask .|. shiftMask, xK_p), spawn $ "variety -p" )
  -- ALT + ... KEYS
...
main = do
        ...
        xmproc0 <- spawnPipe "xmobar -x 0 $HOME/.xmonad/xmobarrc.hs" -- xmobar monitor 1
        xmproc1 <- spawnPipe "xmobar -x 1 $HOME/.xmonad/xmobarrc1.hs" -- xmobar monitor 2
